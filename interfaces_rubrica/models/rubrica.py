...
class InformesRubricas(models.Model):
    ...
    def libro_mayor_pdf(self):
        ...
        pdf.start_page_number = self.rubrica_id.nro_actual
        pdf.company = self.company_id
        pdf.titulo_enc = "Libro Mayor - Periodo: Del {0} al {1}".format(self.fecha_inicio.strftime('%d/%m/%Y'), self.fecha_fin.strftime('%d/%m/%Y'))
        if self.rubrica_id.imagen:
            pdf.rub_img = base64.b64decode(self.rubrica_id.imagen)
        # pdf.set_font("Arial", "B", 10)
        pdf.add_page()
    ...
    def libro_inventario_pdf(self):
                ...
                if force_print_from_report_totals and \
                    ...
                    if release.major_version in ['14.0', '15.0']:
                        report_line_total = sum(
                            ...
                            for expression_id in
                            report_line.search([('id', 'child_of', report_line.ids)]).children_ids.filtered(lambda x: x.domain)
                        )
        ...
        if release.major_version in ['14.0', '15.0']:
            ...
            account_financial_report_bg_l10n_py_report_informations = {x.get('model_ref')[1]: {'value': x.get('columns')[0].get('no_format')} for x in
        ...
        if release.major_version in ['14.0', '15.0']:
            ...
            account_financial_report_er_l10n_py_report_informations = {x.get('model_ref')[1]: {'value': x.get('columns')[0].get('no_format')} for x in
