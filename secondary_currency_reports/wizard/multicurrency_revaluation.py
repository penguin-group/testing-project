...
class MulticurrencyRevaluationWizard(models.TransientModel):
    _name = 'secondary.currency.multicurrency.revaluation.wizard'
    ...
    date = fields.Date(default=lambda self: self._context['secondary_currency_multicurrency_revaluation_report_options']['date']['date_to'], required=True)
    ...
    def default_get(self, default_fields):
        ...
        if 'reversal_date' in default_fields:
            report_options = self._context['secondary_currency_multicurrency_revaluation_report_options']
    ...
    def _compute_preview_data(self):
        ...
        for record in self:
            preview_vals = [self.env['account.move']._move_dict_to_preview_vals(
                self._get_move_vals(),
                record.company_id.sec_currency_id)
            ]
    ...
    def _get_move_vals(self):
        ...
        def _get_adjustment_secondary_balance(line):
            for column in line.get('columns'):
                if column.get('expression_label') == 'adjustment':
                    return column.get('no_format')

        report = self.env.ref('secondary_currency_reports.secondary_currency_multicurrency_revaluation_report')
        ...
        options = {**self._context['secondary_currency_multicurrency_revaluation_report_options'], 'unfold_all': False}
        ...
        for report_line in report._get_unfolded_lines(report_lines, generic_included_line_id):
            ...
            secondary_balance = _get_adjustment_secondary_balance(report_line)
            # parsed_line_id[-1][-2] corresponds to res_model of the current line
            if (
                parsed_line_id[-1][-2] == 'account.account'
                and not self.env.company.sec_currency_id.is_zero(secondary_balance)
            ):
                ...
                move_lines.append(Command.create({
                    ...
                    'debit': secondary_balance if secondary_balance > 0 else 0,
                    'credit': -secondary_balance if secondary_balance < 0 else 0,
                    'amount_currency': 0,
                ...
                move_lines.append(Command.create({
                    ...
                    'amount_currency': 0,
                    'currency_id': currency_id,
                    'account_id': self.expense_provision_account_id.id if secondary_balance < 0 else self.income_provision_account_id.id,
                }))
